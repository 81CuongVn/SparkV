...
module.exports = {
   ...
   */
    init(client) {
        ...
        bot = client;
        canvas_1.default.registerFont(`${path_1.default.join(__dirname, "../../")}/assets/fonts/TheBoldFont.ttf`, { family: "Bold" });
    },
    /**
    ...
    async createCard(options) {
        const canvas = canvas_1.default.createCanvas(800, 390);
        const context = canvas.getContext("2d");
        // Background
        context.fillStyle = "#3461eb";
        context.fillRect(0, 0, canvas.width, canvas.height);
        const background = await canvas_1.default.loadImage(`${path_1.default.join(__dirname, "../../")}/assets/images/background.png`);
        ...
        context.clip();
        /* Avatar */
        const Avatar = await canvas_1.default.loadImage(options.user.displayAvatarURL({ extension: "png" }));
   ...
   */
    async fetchUser(key) {
        ...
        if (match) {
            const user = bot.users.fetch(match[1]).catch(() => { });
            if (user)
                return user;
        }
        const match2 = key.match(/^!?(\w+)#(\d+)$/);
        if (match2) {
            const user = bot.users.cache.find((u) => u.username === match2[0] && u.discriminator === match2[1]);
            if (user)
                return user;
        }
