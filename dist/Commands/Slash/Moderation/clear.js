...
async function execute(bot, message, args, command, data) {
    ...
    if (number < 1 || number > 100)
        return message.replyT(`${bot.config.emojis.success} | Next time, please provide a number greater than 0 and less than 100.`);
    ...
    if (user)
        messages = messages.filter((m) => m.author.id === user.user.id);
    if (type === "pinned")
        messages = messages.filter((m) => !m.pinned);
    if (data.options.getString("content"))
        messages = messages.filter((m) => m.content.toLowerCase().includes(data.options.getString("content").toLowerCase()));
    if (messages.length > number)
        messages.length = parseInt(number, 10);
    number++;
    try {
