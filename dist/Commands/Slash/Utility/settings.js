...
async function execute(bot, message, args, command, data) {
    ...
    async function setNewData(message, options) {
        const requestMsg = new discord_js_1.EmbedBuilder()
            .setAuthor({
            name: message.user.tag,
        ...
        if (options.dropdownItems) {
            ...
            collector.on("collect", async (interaction) => {
                if (interaction.customId.split("_")[1] === options.id) {
                    try {
                        ...
                        try {
                            setTimeout(() => channelMsg.delete(), 5 * 1000);
                        }
                        catch (err) { }
                    }
                    catch (err) {
            ...
            collector.on("end", async () => { try {
                await channelMsg?.edit({ components: [] });
            }
            catch (err) { } });
        }
        else {
            await message.channel.awaitMessages({ filter: (m) => !m.author.id === m.client.user.id, max: 1, time: (options.time * 1000), errors: ["time"] }).then(async (collected) => {
                try {
                    await options.handleData(collected.first(), requestMsg);
                    await channelMsg.edit({ embeds: [requestMsg] });
                    try {
                        setTimeout(() => { channelMsg.delete(); collected.first().delete(); }, 5 * 1000);
                    }
                    catch (err) { }
                }
                catch (err) {
    ...
    /* -------------------------------------------------- SETTINGS --------------------------------------------------*/
    const ToggleButton = new discord_js_1.ButtonBuilder().setLabel("Toggle").setEmoji("⏺️").setCustomId("toggle").setStyle(discord_js_1.ButtonStyle.Danger);
    const channelButton = new discord_js_1.ButtonBuilder().setLabel(await message.translate("Channel")).setEmoji(bot.config.emojis.channel)
        .setCustomId("channel").setStyle(discord_js_1.ButtonStyle.Secondary);
    let settings = [];
    if (data.options.getString("type") === "user") {
        settings = [{
                ...
                buttons: [{
                        ...
                        data: new discord_js_1.ButtonBuilder().setLabel(await message.translate("Reason")).setEmoji(bot.config.emojis.slash).setCustomId("reason").setStyle(discord_js_1.ButtonStyle.Primary),
        ...
        settings = [{
                ...
                buttons: [{
                        ...
                        data: new discord_js_1.ButtonBuilder().setLabel(await message.translate("Language")).setEmoji(bot.config.emojis.slash).setCustomId("language").setStyle(discord_js_1.ButtonStyle.Primary),
                ...
                categories: [{
                        ...
                        emoji: bot.config.emojis.stats,
                        emojiID: "947990408657518652",
                        data: new discord_js_1.ButtonBuilder().setLabel(await message.translate("general")).setEmoji(bot.config.emojis.stats).setCustomId("general").setStyle(discord_js_1.ButtonStyle.Secondary),
                        buttons: [{
                                name: "Toggle",
                                data: ToggleButton,
                                ...
                                setData: async () => {
                                    await setNewData(message, {
                                        ...
                                        time: 60,
                                        handleData: async (collected, requestMsg) => {
                                            if (!collected?.mentions?.channels?.first())
                                                return requestMsg.setTitle(`${bot.config.emojis.config} | Logging Channel Setup`)
                                                    .setDescription(`${bot.config.emojis.alert} Input provided was not a channel.`).setColor(discord_js_1.Colors.Red);
                                            requestMsg
                ...
                buttons: [{
                        ...
                        name: "Message",
                        data: new discord_js_1.ButtonBuilder().setLabel("Message").setEmoji(bot.config.emojis.message).setCustomId("message").setStyle(discord_js_1.ButtonStyle.Danger),
                        ...
                        setData: async () => {
                            await setNewData(message, {
                                ...
                                color: "BLUE",
                                time: 60,
                                filter: async (m) => !m.author.id === m.client.user.id,
                                handleData: async (collected, requestMsg) => {
                                    if (!collected.content)
                                        return await collected.replyT("Dude... I need you to send a message. Not a picture.");
                                    if (collected.content.length >= 500)
                                        await collected.replyT(`${bot.config.emojis.error} | The new leveling message cannot be longer than 500 characters. Try again.`);
                                    requestMsg
                    ...
                    },
                    {
                        ...
                        setData: async () => {
                            await setNewData(message, {
                                ...
                                handleData: async (collected, requestMsg) => {
                                    if (!collected?.mentions?.channels?.first())
                                        return requestMsg.setTitle(`${bot.config.emojis.config} | Leveling Channel Setup`)
                                            .setDescription(`${bot.config.emojis.alert} Input provided was not a channel.`).setColor(discord_js_1.Colors.Red);
                                    requestMsg
                ...
                setState: async (type) => {
                    if (type === "enable")
                        data.guild.leveling.enabled = "true";
                    else if (type === "disable")
                        data.guild.leveling.enabled = "false";
            ...
            },
            {
                ...
                buttons: [{
                        ...
                        name: "Roles",
                        required: true,
                        data: new discord_js_1.ButtonBuilder().setLabel("Support Roles").setEmoji(bot.config.emojis.special).setCustomId("roles").setStyle(discord_js_1.ButtonStyle.Secondary),
                        getData: () => data.guild?.tickets?.roles ? data.guild?.tickets?.roles.map((r) => `<@&${r}>`).join(", ") : "None",
                        setData: async () => {
                            await setNewData(message, {
                ...
                categories: [{
                        ...
                        data: new discord_js_1.ButtonBuilder().setLabel(await message.translate("AntiScam")).setEmoji(bot.config.emojis.warning).setCustomId("antiscam").setStyle(discord_js_1.ButtonStyle.Secondary),
                        buttons: [{
                                name: "Toggle",
                                data: ToggleButton,
                                ...
                                required: true,
                                data: new discord_js_1.ButtonBuilder().setLabel(await message.translate("Actions")).setEmoji(bot.config.emojis.ban).setCustomId("actions").setStyle(discord_js_1.ButtonStyle.Secondary),
                        ...
                        setState: async (type) => {
                            if (type === "enable")
                                data.guild.antiScam.enabled = "true";
                            else if (type === "disable")
                                data.guild.antiScam.enabled = "false";
                        ...
                        data: new discord_js_1.ButtonBuilder().setLabel(await message.translate("AntiSpam")).setEmoji(bot.config.emojis.message).setCustomId("antispam").setStyle(discord_js_1.ButtonStyle.Secondary),
                        buttons: [{
                                name: "Toggle",
                                data: ToggleButton,
                        ...
                        setState: async (type) => {
                            if (type === "enable")
                                data.guild.antiSpam.enabled = "true";
                            else if (type === "disable")
                                data.guild.antiSpam.enabled = "false";
                ...
                buttons: [{
                        ...
                        data: new discord_js_1.ButtonBuilder().setLabel("Channel").setEmoji(bot.config.emojis.channel).setCustomId("channel").setStyle(discord_js_1.ButtonStyle.Secondary),
                        ...
                        setData: async () => {
                            await setNewData(message, {
                                ...
                                handleData: async (collected, requestMsg) => {
                                    if (!collected?.mentions?.channels?.first())
                                        return requestMsg.setTitle(`${bot.config.emojis.config} | Starboard Channel Setup`)
                                            .setDescription(`${bot.config.emojis.alert} Input provided was not a channel.`).setColor(discord_js_1.Colors.Red);
                                    requestMsg
                        ...
                        name: "Emoji",
                        data: new discord_js_1.ButtonBuilder().setLabel("Emoji").setEmoji(bot.config.emojis.star).setCustomId("emoji").setStyle(discord_js_1.ButtonStyle.Secondary),
                        ...
                        name: "Minimum",
                        data: new discord_js_1.ButtonBuilder()
                            .setLabel("Minimum")
    ...
    async function refreshSetting(curSetting) {
        ...
        function handleButton(button) {
            if (button.name.toLowerCase() === "toggle")
                return buttonsIncluded.push(button?.data?.setDisabled(false).setStyle(button.getData() === "true" ? discord_js_1.ButtonStyle.Success : discord_js_1.ButtonStyle.Danger));
            if (curSetting?.stateDisabled === true)
                return buttonsIncluded.push(button?.data?.setDisabled(false));
    ...
    ;
    const Menu = new discord_js_1.EmbedBuilder()
        .setAuthor({
        ...
        .setTimestamp();
    const ExitButton = new discord_js_1.ButtonBuilder().setLabel("Exit").setEmoji(bot.config.emojis.leave).setCustomId("exit").setStyle(discord_js_1.ButtonStyle.Danger);
    const BackButton = new discord_js_1.ButtonBuilder().setLabel("Back").setEmoji(bot.config.emojis.arrows.left).setCustomId("back").setStyle(discord_js_1.ButtonStyle.Secondary);
    const WebsiteButton = new discord_js_1.ButtonBuilder().setURL(`https://${process.env.BASEURL}/`).setEmoji(bot.config.emojis.globe).setLabel("Website").setStyle(discord_js_1.ButtonStyle.Link);
    const SupportButton = new discord_js_1.ButtonBuilder().setURL(bot.config.support).setLabel("Support").setStyle(discord_js_1.ButtonStyle.Link);
    async function createPages() {
        settings.forEach((setting) => {
            ...
            if (setting.category === true) {
                setting.categories.forEach((category) => pages.push({
    ...
    async function setupSettings() {
        settings.forEach(async (setting) => {
            const SettingButton = new discord_js_1.ButtonBuilder()
    ...
    const collector = botMessage.createMessageComponentCollector({
        filter: async (interaction) => {
            if (interaction.user.id !== message.user.id)
                await interaction.reply({ content: `Only ${message.user} can edit these settings!`, ephemeral: true });
            return interaction.user.id === message.user.id;
        }, time: 300 * 1000
    });
    collector.on("collect", async (interaction) => {
        ...
        try {
            ...
            switch (interaction.customId) {
                ...
                case "toggle": {
                    if (curSetting && curSetting?.buttons?.find((button) => button?.name?.toLowerCase() === "toggle")) {
                        if ((curSetting.getState() === "true" ? "false" : "true") === "true") {
                            await curSetting.setState("enable");
                            curSetting?.buttons?.forEach(async (button) => button?.required === true && await button.setData());
                        }
                        else {
                ...
                default: {
                    if (curSetting?.buttons?.find((button) => button?.data?.data?.custom_id?.toLowerCase() === interaction?.customId?.toLowerCase())) {
                        await curSetting?.buttons?.find((button) => button.data.data.custom_id.toLowerCase() === interaction.customId.toLowerCase()).setData();
                        return refreshSetting(curSetting);
                    }
                    let foundSetting;
                    settings.forEach(setting => {
                        if (setting?.category === true) {
                            if ((setting.id.toLowerCase() === interaction.customId.toLowerCase()) === true)
                                foundSetting = setting;
                            if (setting?.categories.find((cat) => cat.id.toLowerCase() === interaction.customId.toLowerCase()))
                                foundSetting = setting?.categories.find((cat) => cat.id.toLowerCase() === interaction.customId.toLowerCase());
                        }
                        if ((setting.id.toLowerCase() === interaction.customId.toLowerCase()) === true)
                            foundSetting = setting;
                    });
