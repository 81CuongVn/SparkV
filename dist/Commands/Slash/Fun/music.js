...
async function execute(bot, message, args, command, data) {
    if (!message?.member?.voice?.channel)
        return message.replyT(`${bot.config.emojis.alert} | You must be in a voice channel to use this command.`);
    ...
    if (state === "song") {
        ...
        if (!playerData)
            return message.replyT(`${bot.config.emojis.alert} | There is nothing in the queue right now!`);
        if (type === "volume") {
            if (number > 100)
                return message.replyT(`${bot.config.emojis.alert} | Songs cannot go louder than 100%.`);
            playerData.setVolume(number);
            return await message.replyT({ embeds: [embed.setDescription(`${bot.config.emojis.music} | The volume is now set to ${number}%.`).setColor(discord_js_1.ButtonStyle.Success)] });
        }
            ...
            return await message.replyT({ embeds: [embed.setDescription(`${bot.config.emojis.music} | I forwarded the song by ${number} seconds ahead.`).setColor(discord_js_1.ButtonStyle.Success)] });
        }
        else if (type === "rewind") {
            let rewind = playerData.queue.currentTime - number;
            7;
            if (rewind < 0)
                rewind = 0;
            ...
            return await message.replyT({
                embeds: [embed.setDescription(`${bot.config.emojis.music} | I rewinded the song by ${number} seconds.`).setColor(discord_js_1.ButtonStyle.Success)]
            });
        }
    }
    else if (state === "play") {
        ...
        if (!track)
            return await message.replyT(`${bot.config.emojis.alert} | No results found for that query.`);
        ...
        if (!player.playing && !player.paused && !player.queue.size)
            player.play();
        if (!player.playing && !player.paused && player.queue.totalSize === queryData.tracks.length)
            player.play();
        // If the track is the first song in the queue, don't send the message.
        if (player?.queue?.size > 0) {
            const playerData = bot.music.players.get(message.guild.id);
            const SongAddedQueue = new discord_js_1.default.EmbedBuilder()
        ...
        const query = data.options.getString("search");
        if (!query)
            return message.replyT(`${bot.config.emojis.error} | Please supply the title of a song to search for.`);
        let lyrics;
        try {
        ...
        if (!lyrics)
            return await message.replyT(`${bot.config.emojis.error} | I couldn't find the lyrics for **${query}**!`);
        ...
        collector.on("collect", async (interaction) => {
            ...
            if (interaction.customId === "quickLeft")
                ...
                PageNumber > 0 ? --PageNumber : PageNumber = (pages.length - 1);
            else if (interaction.customId === "right")
                PageNumber + 1 < pages.length ? ++PageNumber : PageNumber = 0;
        ...
        if (type === "join") {
            ...
            if (!playerData)
                return message.replyT(`${bot.config.emojis.alert} | There is nothing in the queue right now!`);
            const queueSongs = playerData?.queue?.songs?.map((song, id) => `${Emotes[id] || (id + 1)} **${song.title}** - ${bot.music.formatDuration(song.duration)}`).slice(0, 10);
            const queueEmbed = new discord_js_1.default.EmbedBuilder()
                .setAuthor({
                name: message.user.tag,
                iconURL: message.user.displayAvatarURL()
            })
            ...
            embed
                ...
                },
                {
                    ...
                    value: `\`${playerData?.queue?.map((song, id) => `${Emotes[id] || (id + 1)} **${song.title}** - ${bot.music.formatDuration(song.duration)}`).slice(0, 10)}\``,
                    inline: false
                },
                {
                    name: `${bot.config.emojis.volume} Volume`,
            ...
            if (!queue)
                return await message.replyT(`${bot.config.emojis.error} | No songs in queue.`);
            if (!queue.previousSongs || queue.previousSongs.length === 0)
                return await message.replyT(`${bot.config.emojis.error} | There are no previous songs.`);
