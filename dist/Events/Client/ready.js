...
const dbots_1 = __importDefault(require("dbots"));
exports.default = {
    once: true,
    async execute(bot) {
        ...
        if (process.argv.includes("--dev") === false) {
            ...
            if (process.env.DBLKEY)
                apiKeys.topgg = process.env.DBLKEY;
            if (process.env.VBLKEY)
                apiKeys.voidbots = process.env.VBLKEY;
            if (process.env.DLBLKEY)
                apiKeys.discordlabs = process.env.DLBLKEY;
            if (process.env.DBGGKEY)
                apiKeys.discordbotsgg = process.env.DBGGKEY;
            if (process.env.DBLEUKEY)
                apiKeys.DiscordBotlistEU = process.env.DBLEUKEY;
            const poster = new dbots_1.default.Poster({
                client: bot,
                apiKeys,
