...
const cursewords_json_1 = __importDefault(require("../../cursewords.json"));
const messages = [];
// Timeout user
function timeoutUser(offense, message, data) {
    ...
    message.member.timeout((10 * data.member.infractionsCount) * 1000, `Placed on timeout for ${message.client.functions.MSToTime((10 * data.member.infractionsCount) * 1000)} for ${offense}.`)
        .then(async () => {
        const timeoutEmbed = new discord_js_1.default.EmbedBuilder()
            .setAuthor({
...
exports.default = {
    ...
    async execute(bot, message) {
        ...
        if (!botMember.permissionsIn(message.channel).has("SendMessages"))
            return;
        // If the guild is part of the guild blacklist, return.
        if (bot.config.blacklist.guilds[message.guild.id])
            return await message.replyT(`Your server has been blacklisted. Reason: ${bot.config.blacklist.guilds[message.guild.id]}\n\nIf you think this ban wasn't correct, please contact support. (https://discord.gg/PPtzT8Mu3h)`);
        // Cache the member.
        ...
        if (message.guild && message?.author?.id && message?.guild?.id)
            data.member = await bot.database.getMember(message?.author?.id, message?.guild?.id);
        // User data
        ...
        if (message.guild) {
            ...
            message.mentions.users.forEach(async (u) => {
                ...
                if (mentionedUserData?.afk?.enabled === "true")
                    await message.replyT({
                        embeds: [{
                                author: { name: message.author.tag, icon_url: message.author.displayAvatarURL() },
            ...
            if (data.guild?.antiScam.enabled === "true") {
                if (!message.channel.permissionsFor(message.member).has("ManageMessages")) {
                    ...
                    if (!scamLinks) {
                        scamLinks = await axios_1.default.get("https://phish.sinking.yachts/v2/all").then(res => res.data).catch(() => message.replyT("Failed to fetch scam links."));
            ...
            if (data.guild.automod.removeProfanity === "true") {
                if (!message.channel.permissionsFor(message.member).has("ManageMessages")) {
                    ...
                    let cursed = false;
                    for (var i in cursewords_json_1.default) {
                        if (message.content.toLowerCase().includes(cursewords_json_1.default[i].toLowerCase()))
                            cursed = true;
                    }
        ...
        if (bot.config.blacklist.users[message.author.id])
            return await message.replyT(`You have been blacklisted. Reason: ${bot.config.blacklist.users[message.author.id]}\n\nIf you think this ban wasn't correct, please contact support. (https://discord.gg/PPtzT8Mu3h)`);
        // Check for a prefix
        const prefix = process.argv.includes("--dev") === true ? "_" : "sv!";
        if (!message.content.toLowerCase().startsWith(prefix) && message.content.match(new RegExp(`^<@!?${bot.user.id}>( |)$`)))
            return message.replyT(`**Hi there!**\nPlease run \`/help\` to see what I can do. \It took me ${new Date().getTime() - message.createdTimestamp}ms to send this message.`);
